...
import com.raffaeleconforti.splitminer.splitminer.dfgp.variants.DFGPFilterWithGuarantees;
import com.raffaeleconforti.splitminer.splitminer.dfgp.variants.DFGPFilterWithThreshold;
import com.raffaeleconforti.splitminer.splitminer.dfgp.variants.DFGPNoFilter;
import com.raffaeleconforti.splitminer.splitminer.dfgp.variants.DFGPStandardFilter;
import com.raffaeleconforti.splitminer.splitminer.oracle.Oracle;
import com.raffaeleconforti.splitminer.splitminer.oracle.OracleItem;
...
public class SplitMiner {
    ...
    public DirectlyFollowGraphPlus getDFGP() {
        return dfgp;
    }
    ...
    public BPMNDiagram mineBPMNModel(SimpleLog log, DirectlyFollowGraphPlus dfgp, SplitMinerUIResult.StructuringTime structuringTime) {
...
//        System.out.println("SplitMiner - log parsed successfully");
        ...
        filterType = percentileFrequencyThreshold == 0 ? DFGPUIResult.FilterType.NOF : filterType;
        switch (filterType) {                        //depends on detectParallelisms()
            case FWG:
                dfgp = new DFGPFilterWithGuarantees(this.log, percentileFrequencyThreshold, parallelismsThreshold, parallelismsFirst);
                break;
            case WTH:
                dfgp = new DFGPFilterWithThreshold(this.log, percentileFrequencyThreshold, parallelismsThreshold, parallelismsFirst);
                break;
            case STD:
                dfgp = new DFGPStandardFilter(this.log, percentileFrequencyThreshold, parallelismsThreshold, parallelismsFirst);
                break;
            case NOF:
                dfgp = new DFGPNoFilter(this.log, percentileFrequencyThreshold, parallelismsThreshold, parallelismsFirst);
                break;
        }
        generateDFGP();
        try {
