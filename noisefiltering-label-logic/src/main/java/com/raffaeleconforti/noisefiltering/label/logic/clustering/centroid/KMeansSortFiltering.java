 ...
 */
public class KMeansSortFiltering<T> extends AbstractClustering<T> {

    public KMeansSortFiltering(T[] key, double[][] data, int maxClts) {
    ...
    public List<FilteringResult<T>> getOutliers() {
        ...
        for (int l = 0; l < kMeansInitializations.length; l++) {
            ...
            for (int d = 0; d < distanceFunctions.length; d++) {
                ...
                ClusteringAlgorithm<Clustering<PrototypeModel>> clusteringMethod = new KMeansSort(
                        distanceFunction,
