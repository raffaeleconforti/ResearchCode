 ...
 */
public class KMeansMacQueenFiltering<T> extends AbstractClustering<T> {

    public KMeansMacQueenFiltering(T[] key, double[][] data, int maxClts) {
    ...
    public List<FilteringResult<T>> getOutliers() {
        ...
        for (int l = 0; l < kMeansInitializations.length; l++) {
            ...
            for (int d = 0; d < distanceFunctions.length; d++) {
                ...
                ClusteringAlgorithm<Clustering<PrototypeModel>> clusteringMethod = new KMeansMacQueen(
                        distanceFunction,
                ...
                try {
                    String technique = "KMeansMacQueen - (" +
