package com.raffaeleconforti.keithshwarz.algorithms.maximummatchings;
 ...
 *
 * A collections of alternating paths can be formed together to create an
 ...
 * alternating tree is then a collections of paths formed from these gadgets
...
import com.raffaeleconforti.keithshwarz.datastructure.graph.undirectedgraph.edmonds.UndirectedGraphEdmonds;
...
public final class EdmondsMatching {
     ...
     */
    private static <T> void updateMatching(List<T> path, UndirectedGraphEdmonds<T> m) {
    ...
    /* A key step of the algorithm works by building up a collections of
     ...
     */
    private static <T> List<T> findAlternatingPath(UndirectedGraphEdmonds<T> g,
                                                   UndirectedGraphEdmonds<T> m) {
         ...
         */
        Map<T, NodeInformation<T>> forest = new UnifiedMap<T, NodeInformation<T>>();
    ...
    private static <T> Blossom<T> findBlossom(Map<T, NodeInformation<T>> forest,
        ...
        return new Blossom<T>(onePath.get(mismatch - 1), cycle, new UnifiedSet<T>(cycle));
    }
     ...
     */
    private static <T> UndirectedGraphEdmonds<T> contractGraph(UndirectedGraphEdmonds<T> g,
                                                               Blossom<T> blossom) {
     ...
     *         node.
     */
    private static <T> T findNodeLeavingCycle(UndirectedGraphEdmonds<T> g,
                                              Blossom<T> blossom,
                                              T node) {
