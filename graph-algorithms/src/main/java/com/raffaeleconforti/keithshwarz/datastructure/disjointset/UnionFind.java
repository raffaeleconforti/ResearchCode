package com.raffaeleconforti.keithshwarz.datastructure.disjointset;
...
public final class UnionFind<T> {
     ...
     * rank and parent.
     */
    private final Map<T, Link<T>> elems = new UnifiedMap<T, Link<T>>();
    ...
    public UnionFind(Collection<? extends T> elems) {
        /* Iterate across the collections, adding each to this structure. */
        for (T elem: elems)
            add(elem);
    }
